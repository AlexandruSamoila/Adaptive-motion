...
#         self.qd = 3 # Value chosen arbitrarily
...
#         # 
#         # self.pos_des=np.zeros(6)
#         # self.vel_des=np.zeros(6)
        
#         # self.alpha=24
#         # self.n_bfs=6
#         # self.cs = CanonicalSystem(alpha=self.alpha)
...
#         # def features(xj):
#         #     w=np.exp(-self.h * (xj - self.c)**2)
#         #     return w/w.sum()
        
#         # g=np.stack(features(xj) for xj in x)
...
#             self.pos = np.array([pos[0, 0],pos[0, 1],pos[0, 2],pos[0, 0],pos[0, 1],pos[0, 2]])
#             self.vel = np.array([vel_p[0, 0],vel_p[0, 1],vel_p[0, 2],vel_p[0, 0],vel_p[0, 1],vel_p[0, 2]])
...
#                 self.pos_des[3],self.pos_des[4],self.pos_des[5] = pos[j, 0],pos[j, 1],pos[j, 2] 
#                 self.vel_des[0]= vel_p[j, 0]
#                 self.vel_des[1]= vel_p[j, 1]
#                 self.vel_des[2] = vel_p[j, 2] 
#                 self.vel_des[3]= vel_p[j, 0]
#                 self.vel_des[4]= vel_p[j, 1]
#                 self.vel_des[5] = vel_p[j, 2] 
...
#                 if j == 0:
#                     self.vel_des = np.zeros(6) #But WHY GODDAMNIT! Ask supervisor
                
#                 g = 0.25 # Need from DMP!
...
class Ada_con():
    ...
    def __init__(self, dof, tr, t):
        ...
        # Adaptation rates for stiffness and damping
        self.qs = 3 # Value chosen arbitrarily
        self.qd = 2 # Value chosen arbitrarily
        ...
        # Forgetting factor
        self.lambd = 0.5 # Chosen arbitrarily
    ...
    def mass_spring_damper(self):
        ...
        #self.acceleration = -(self.spring_force + self.damper_force) / self.mass + self.noise
        # + or - self.tau in the equation above?
        self.vel = self.vel + self.acceleration * self.dt1[0]
        self.pos = self.pos + self.vel * self.dt1[0]
    ...
    def iter_learn(self, pos, vel_p):
        ...
        for i in range(self.nt):
            ...
            for j in range(self.samples):
                ...
                self.pos_des=pos[j, 0]
                self.vel_des = (self.pos_des - self.pos_old) / self.dt1[j]
