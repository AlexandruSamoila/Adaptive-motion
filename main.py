...
from data_processing import extractData
...
def learnParameters(dof, qs, qd, qv, gamma, demo_p, actual_vel, force,actual_acceleration):
    """
    ...
    for j in range(len(dmp_p)):
        ...
        #tau1[j, :] = force[j, :]
        acceleration = (force[j, :] - ks[j, :] * pos_diff - kd[j, :] * vel_diff) / mass + actual_acceleration[j,:]
...
if __name__ == '__main__':
    ...
    ks, kd, v, tau1 = learnParameters(dof=dof, qs=qs, qd=qd, qv=qv, gamma=gamma, demo_p=demo_p, actual_vel=actual_velocity_p, force=force_p,actual_acceleration=dmp_ddp)

    tr = 1# Trials

    ac = Ada_con(dof=dof, tr=tr, t=t)
    ac.iter_learn(pos=dmp_p, vel_p=dmp_dp, ks=ks, kd=kd, v=v, force=force_p,actual_acceleration=dmp_ddp)
    ...
    #         rtde_c.servoL(servo_target, 0, 0, dt, lookahead_time, gain)
